...
async def google_callback(
    ...
    - **ref**: Optional referral code for tracking referrals
    """
    logger.info(f"Google OAuth callback received: code={code[:20]}...")
    
    try:
        # Extract referral code from query parameter or state
        referral_code = ref or (state if state and state.startswith("ARTEMIS-") else None)
